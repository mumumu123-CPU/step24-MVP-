...
- 緑を基調にしたデザインにこだわりました。緑は「安心」「回復」といったイメージを持つ色で、初めて精神科を探す方にも優しい印象になるよう意識しました。
- APIによる実データの取得は難しかったため、実際の病院情報にできるだけ近い**リアルなダミーデータ**を作成しました。
- 精神科は「どんな病気を診ているか」が病院ごとにバラバラなため、**専門外来・疾患・都道府県**を組み合わせて探せる検索バーを設けました。
